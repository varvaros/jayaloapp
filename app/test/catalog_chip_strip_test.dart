...
void main() {
  ...
  testWidgets('el chip activo lleva selected=true en semántica', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
  ...
  testWidgets('sin wholesale no hay chip de mayoreo (Servicio)', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(
  ...
  testWidgets('con categoría y rubros pinta la fila de rubros y avisa', (
    tester,
  ) async {
    String? r = 'x';
    await tester.pumpWidget(
      host(
        CatalogChipStrip(
          categorias: const [(id: 'salud', name: 'Salud')],
          categoryId: 'salud',
          rubros: const ['Medicina general', 'Odontología'],
          rubro: null,
          onRubro: (v) => r = v,
          onCategory: (_) {},
          onTodo: () {},
