...
void main() {
  ...
  group('ProviderStoreView — una sola página, sin pestañas (PO 2026-08-09)', () {
    testWidgets(
      '(a) sin HeaderSegmented y las 4 secciones se pintan a la vez',
      (tester) async {
    ...
    testWidgets(
      ...
      'Añadir…» y mantener presionado no dispara nada',
      (tester) async {
        setTallPhoneSize(tester);
        await tester.pumpWidget(
          host(
    ...
    testWidgets('(c) las 4 secciones vacías: un único aviso, no cuatro', (
      tester,
    ) async {
      setTallPhoneSize(tester);
      await tester.pumpWidget(
      ...
      expect(find.text('TRABAJOS'), findsNothing);
      expect(
        find.text('Este proveedor aún no publica nada en su tienda.'),
        findsOneWidget,
      );
    });
  });
  ...
  // «Pedir cotización» (pedido PO 2026-09-20, paridad con la tienda web
  // `business.$id.tsx`): oculto al dueño y sin sesión (onCotizar null).
  group('ProviderStoreView — «Pedir cotización» (paridad web)', () {
    testWidgets(
      '(c) con onCotizar pinta «Pedir cotización» bajo la portada y llama al toque',
      (tester) async {
        setTallPhoneSize(tester);
        var toques = 0;
        await tester.pumpWidget(
        ...
        expect(
          find.textContaining('Ferretería Pérez te manda una oferta'),
          findsOneWidget,
        );
        await tester.tap(find.text('Pedir cotización'));
        expect(toques, 1);
      },
    );

    testWidgets('(d) sin onCotizar (dueño o sin sesión) no hay botón', (
      tester,
    ) async {
      setTallPhoneSize(tester);
      await tester.pumpWidget(
